...
@app.route('/news')
def news():
    #dbase.addNews('НАЗВАНИЕ', 'static/images/logo.jpg', 'fsfs', 'team', 'fsfsdfsfsdfsdfs') #название категории с маленькой буквы
...
def admin():
    ...
    if form.validate_on_submit():
        ...
        dbase.addNews(form.title.data, form.image_url.data, form.short_description.data, form.category.data, form.full_text.data)
        flash('Новость добавлена!', 'success')
        return redirect(url_for('admin'))
    
    # Получаем список всех новостей для отображения
    news_list = dbase.getAllNews()
    return render_template('admin.html', form=form, news_list=news_list)
